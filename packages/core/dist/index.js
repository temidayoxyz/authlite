...
var AuthLite = class {
  ...
  async signUp(email, password) {
    try {
      ...
      const passwordHash = await (0, import_argon2.hash)(password);
  ...
  async signIn(email, password) {
    try {
      ...
      const isValid = await (0, import_argon2.verify)(user.passwordHash, password);
      if (!isValid) {
        return { success: false, error: "Invalid credentials" };
      }
  ...
  hashToken(token) {
    return (0, import_node_crypto.createHash)("sha256").update(token + this.config.secret).digest("hex");
  }
};
